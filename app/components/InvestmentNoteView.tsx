    ...
    <div className="investment-note">
      ...
      <NoteSectionWrapper number="4." title="Transaction Features" compactMode={compactMode} collapseInCompact={true}>
        ...
        {/* MODULE 3 DD TECHNIQUE
            Audit deterministe du depot GitHub via API REST v3, dix
            tests structures avec evidence chiffree, severity descriptive
            et questions DD ciblees. Ne s affiche que si triggered (URL
            fournie ET depot lu avec succes). Si reasonNotTriggered est
            renseigne, on affiche un bandeau d explication minimal pour
            que le partner sache pourquoi le moteur n a pas tourne. */}
        {(() => {
          const ddt = (r as any).ddTechnical;
          if (!ddt) return null;
          if (!ddt.triggered) {
            // Si l URL a ete fournie mais le fetch a echoue, afficher
            // une note d explication. Sinon ne rien afficher.
            if (ddt.audit?.repoUrl) {
              return (
                <>
                  <h3 className="note-h3">Data Room - DD Technique</h3>
                  <div className="dd-not-triggered" style={{ padding: '12px 16px', background: '#faf3ec', border: '1px solid #c4a484', fontSize: 13, lineHeight: 1.55 }}>
                    {ddt.reasonNotTriggered || 'Audit GitHub non realise.'}
                  </div>
                </>
              );
            }
          ...
          const v = ddt.vitals;
          const tests = [
            { key: 'releaseCadence', t: ddt.tests.releaseCadence },
            { key: 'commitCadence', t: ddt.tests.commitCadence },
            { key: 'repoFreshness', t: ddt.tests.repoFreshness },
            { key: 'issuesDiscipline', t: ddt.tests.issuesDiscipline },
            { key: 'pullRequestsDiscipline', t: ddt.tests.pullRequestsDiscipline },
            { key: 'continuousIntegration', t: ddt.tests.continuousIntegration },
            { key: 'basicSecurity', t: ddt.tests.basicSecurity },
            { key: 'documentation', t: ddt.tests.documentation },
            { key: 'busFactor', t: ddt.tests.busFactor },
            { key: 'dependencies', t: ddt.tests.dependencies },
          ];
          return (
            <>
              <h3 className="note-h3">Data Room - DD Technique</h3>
              <p className="note-paragraph muted">
                Audit du depot {repo?.fullName}{repo?.primaryLanguage ? `, ${repo.primaryLanguage}` : ''}{repo?.license ? `, license ${repo.license}` : ''}.
                {' '}Visibilite {repo?.visibility}, {repo?.stars || 0} stars, {repo?.forks || 0} forks.
                {repo?.archived ? ' Depot archive.' : ''}
                {repo?.isFork ? ' Depot fork.' : ''}
                {' '}Score global de l&apos;audit : {ddt.globalScore}/100.
                {!ddt.authenticated && ' Audit realise sans token, certains champs securite restent non observables.'}
              </p>
              ...
              {/* Vitals : indicateurs cles consolides */}
              <div className="dd-vitals-grid">
                {[
                  { label: 'Commits 12 sem.', value: v.last12Weeks !== null ? String(v.last12Weeks) : 'n.a.' },
                  { label: 'Contributeurs', value: v.contributorsTotal !== null ? `${v.contributorsTotal}${v.activeContributorsRecent !== null ? ` (${v.activeContributorsRecent} actifs)` : ''}` : 'n.a.' },
                  { label: 'Bus factor top', value: v.busFactorTopShare !== null ? `${Math.round(v.busFactorTopShare * 100)}%` : 'n.a.' },
                  { label: 'Releases 12 mois', value: v.releasesLast365 !== null ? String(v.releasesLast365) : 'n.a.' },
                  { label: 'Issues O/F', value: v.issuesOpen !== null && v.issuesClosed !== null ? `${v.issuesOpen}/${v.issuesClosed}` : 'n.a.' },
                  { label: 'PR merge median', value: v.medianPRTimeToMergeDays !== null ? `${v.medianPRTimeToMergeDays} j` : 'n.a.' },
                  { label: 'CI succes', value: v.ciSuccessRate !== null ? `${Math.round(v.ciSuccessRate * 100)}%` : 'n.a.' },
                  { label: 'Dernier push', value: v.daysSinceLastPush !== null ? `il y a ${v.daysSinceLastPush} j` : 'n.a.' },
                ].map((kv, i) => (
                  <div key={i} className="dd-vital-cell">
                    <div className="dd-vital-label">{kv.label}</div>
                    <div className="dd-vital-value">{kv.value}</div>
                  </div>
                ))}
              </div>

              {/* Dix tests cote a cote, meme markup que ddf et ddc
                  pour heriter des styles dd-tests / dd-test-row */}
              <div className="dd-tests">
                {tests.map((row) => {
                  const t = row.t;
                  if (!t) return null;
                  return (
                    <div key={row.key} className={`dd-test-row dd-test-${t.severity}`}>
                      <div className="dd-test-header">
                        <span className="dd-test-id">{t.testId}</span>
                        <span className="dd-test-label">{t.testName}</span>
                        <span className={`dd-test-pill dd-test-pill-${t.severity}`}>
                          {t.severity === 'aligned' && 'Aligne'}
                          {t.severity === 'attention' && 'Attention'}
                          {t.severity === 'alert' && 'Alerte'}
                          {t.severity === 'red_flag' && 'Red flag'}
                          {t.severity === 'not_assessable' && 'Non observable'}
                        </span>
                      </div>
                      <div className="dd-test-values">
                        <div><strong>Observe</strong>{t.observation}</div>
                        <div><strong>Standard</strong>{t.benchmark}</div>
                      </div>
                      {t.evidence && <div className="dd-test-evidence">{t.evidence}</div>}
                      {t.ddQuestion && <div className="dd-test-question"><strong>Question DD</strong>{t.ddQuestion}</div>}
                    </div>
                  );
                })}
              </div>
              ...
              <div className={`dd-verdict dd-verdict-${ddt.verdict}`}>
                <strong>Verdict :</strong>{' '}
                {ddt.verdict === 'tech_strong' && 'Discipline d ingenierie elevee. Confirmer en entretien CTO la robustesse du processus interne.'}
                {ddt.verdict === 'tech_solid' && 'Projet correct avec quelques zones d ombre mineures. Audit complementaire sur les points d attention identifies.'}
                {ddt.verdict === 'tech_partial' && 'Signaux mixtes. Certaines dimensions sont solides, d autres incompletes ou non observables. Investigation ciblee requise.'}
                {ddt.verdict === 'tech_concerns' && 'Plusieurs alertes sur la maintenabilite ou la securite. Documenter les zones rouges aupres du CTO avant comite d investissement.'}
                {ddt.verdict === 'tech_red_flags' && 'Red flags structurels identifies (depot abandonne, securite absente, ou alertes critiques ouvertes). Clarification urgente requise.'}
                {ddt.verdict === 'not_applicable' && 'Audit non realisable.'}
              </div>

              {ddt.criticalAlerts?.length > 0 && (
                <div className="dd-questions" style={{ borderLeftColor: '#a83232' }}>
                  <strong>Alertes critiques :</strong>
                  <ol>{ddt.criticalAlerts.map((a: string, i: number) => <li key={i}>{a}</li>)}</ol>
                </div>
              )}
      ...
      <style jsx>{`
        ...
        */
        ...
        /* DD TECHNIQUE - verdicts module 3.
           Cinq niveaux paralleles aux verdicts ddf/ddc, avec un
           code couleur identique pour faciliter la lecture
           transverse Bloc 2 par le partner. */
        ...
        /* DD TECHNIQUE - grille de vitaux (8 indicateurs cles
           consolides en haut de section). Lecture rapide des KPIs
           du depot avant les dix tests detailles. */
        .dd-vitals-grid {
          display: grid;
        ...
        @media (max-width: 700px) {
          .dd-vitals-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        }
